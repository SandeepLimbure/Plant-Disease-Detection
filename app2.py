...
# Prediction Page
    ...
    if test_image is not None:
        st.image(test_image, width=400)  # Display uploaded image

    # Predict button
    if st.button("Predict") and test_image is not None:
        st.snow()  # Snow effect for better UI
        st.write("Prediction in progress...")

        # Make prediction
        result_index = model_prediction(test_image)

        # Class names (same as the model's output classes)
